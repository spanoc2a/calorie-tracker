...
export default function App() {
  ...
  useEffect(() => {
    ...
    const promises = Array.from({ length: 7 }, (_, i) => {
      ...
      return fetch(`/api/entries?date=${dateKey(d)}`)
        ...
        .then(data => ({
          day: DAYS[d.getDay()],
          kcal: (data.entries || []).reduce((a, e) => a + e.kcal, 0)
        }));
    });
  ...
  async function removeEntry(id) {
    await fetch('/api/entries', {
      method: 'DELETE',
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ date: key, id }),
    });
      ...
      <div className="app">
          ...
          <>
            ...
            {pendingItems.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <button className="btn secondary" style={{ width: "100%" }} onClick={() => setSaveModal(true)}>
        ...
        {tab === "stats" && (
          <>
                ...
                {weekData.map((d, i) => (
                  <div className="bar-wrap" key={i}>
                    ...
                    <div className="bar" style={{
                      ...
                      background: d.kcal > goal ? "#c87070" : d.kcal > goal * 0.85 ? "#c8a060" : "#7abf8a"
                    }} />
            ...
            <div className="macros">
              {[
                { label: "Moy. calories", val: weekData.length ? Math.round(weekData.reduce((a,d)=>a+d.kcal,0)/weekData.filter(d=>d.kcal>0).length||0) : 0, unit: "kcal" },
