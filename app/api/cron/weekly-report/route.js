...
${prevCtx}`;
  ...
  const entry = {
    id: Date.now(),
    title: pushText(lang, 'weekly_report_entry_title', { date: startDate }),
    days: 7,
    date: endDate,
    html,
    type: 'hebdomadaire',
  ...
  await sendExpoPushToUser(userId, pushText(lang, 'weekly_report_ready_title'), pushText(lang, 'weekly_report_ready_body'), { type: 'report' });
  return entry;
}
...
export async function GET(req) {
  ...
  return runBatchedCron(req, 'weekly-report', {
    batch: 15,
    chunk: 5,
    filter: (u) => u.role === 'athlete' || u.role === 'Particulier' || !u.role || u.role === 'user',
    handler: (user) => generateWeeklyReport(user.id),
  });
}
